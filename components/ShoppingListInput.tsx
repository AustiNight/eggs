...
const ShoppingListInput: React.FC<ShoppingListInputProps> = ({ items, setItems, onStartSearch }) => {
  ...
  // Force reload history when opening the panel
  useEffect(() => {
    if (showHistory) {
  ...
  const addSelectedHistory = () => {
    const selectedItems = historyItems.filter(h => selectedHistoryIds.has(h.id));
    const newItems = selectedItems.map(h => ({
      id: generateId(),
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex gap-2">
        <form onSubmit={(e) => handleAddItem(e)} className="relative grow">
          <input
            type="text"
            value={inputValue}
               ...
               <div className="max-h-60 overflow-y-auto custom-scrollbar space-y-2 pr-2">
                {historyItems.map((hItem) => {
                  ...
                  return (
                    ...
                    >
                      {isSelected ? <CheckSquare className="w-5 h-5 text-blue-400 shrink-0" /> : <Square className="w-5 h-5 text-slate-600 shrink-0" />}
                      <div className="flex-1 min-w-0">
                        <div className="text-slate-200 font-medium truncate">{hItem.clarifiedName || hItem.name}</div>
                        <div className="text-xs text-slate-500">Last bought: {new Date(hItem.lastPurchased || '').toLocaleDateString()}</div>
                      </div>
        ...
        <div className="bg-slate-800/50 rounded-xl p-4 min-h-[100px] border border-slate-700/50">
          <div className="flex flex-wrap gap-2">
            {items.map((item) => (
              <div 
                key={item.id} 
                className="group flex items-center gap-2 bg-slate-700 text-slate-200 px-3 py-1.5 rounded-lg border border-slate-600 hover:border-amber-400/50 transition-colors animate-fadeIn"
              >
      ...
      <div className="flex justify-center pt-4">
        <button
          ...
          className={`
            flex items-center gap-3 px-8 py-4 rounded-full font-bold text-lg shadow-lg shadow-amber-900/20 transform transition-all
            ${items.length === 0 
              ? 'bg-slate-700 text-slate-500 cursor-not-allowed' 
              : 'bg-gradient-to-r from-amber-400 to-orange-500 text-slate-900 hover:scale-105 active:scale-95 hover:shadow-orange-500/20'}
          `}
        >
