...
const ClarificationModal: React.FC<ClarificationModalProps> = ({ requests, onComplete }) => {
  // Store the user's selected clarifications: itemId -> Set of selected strings
  const [selections, setSelections] = useState<Record<string, Set<string>>>({});

  const handleToggle = (itemId: string, option: string) => {
    setSelections(prev => {
      const itemSelections = new Set(prev[itemId] || []);
      if (itemSelections.has(option)) {
        itemSelections.delete(option);
      } else {
        itemSelections.add(option);
      }
  ...
  const handleSubmit = () => {
    ...
    Object.entries(selections).forEach(([itemId, selectedSet]) => {
      const currentSet = selectedSet as Set<string>;
      if (currentSet.size > 0) {
        // Retrieve original name if possible, or just append options
        // We will construct the "clarifiedName" here
        const originalName = requests.find(r => r.itemId === itemId)?.originalName || '';
        const details = Array.from(currentSet).join(', ');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 animate-fadeIn">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-xl rounded-2xl shadow-2xl p-6 overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="space-y-8 overflow-y-auto pr-2 custom-scrollbar grow">
          {requests.map((req, idx) => (
            <div key={idx} className="space-y-3 animate-slideIn" style={{ animationDelay: `${idx * 100}ms` }}>
                ...
                {req.options.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        px-4 py-2.5 rounded-lg text-sm transition-all border flex items-center gap-2
                        ${isSelected 
        ...
        <div className="mt-8 pt-4 border-t border-slate-800 flex justify-between items-center shrink-0">
          ...
          <button
            ...
            className={`
              ...
              ${isComplete 
                ? 'bg-amber-400 text-slate-900 hover:bg-amber-300 hover:scale-105 shadow-lg shadow-amber-500/20' 
